...
# Discover the WAN slave's hardware MAC. br0's hwaddress is pinned to this
# value so the appliance presents the same Layer 2 identity to the network
# after bring-up as before — preventing mDNS host-name conflicts (avahi
# falling back to xboxlive-protect-2.local) caused by stale router/mDNS
# caches seeing two MACs claim the same hostname.
WAN_MAC="$(cat /sys/class/net/${WAN_IFACE}/address 2>/dev/null)"
[[ -n "$WAN_MAC" ]] || \
    die "Could not read MAC address from /sys/class/net/${WAN_IFACE}/address.
  Is ${WAN_IFACE} a valid network interface?"
info "WAN MAC for br0 hwaddress: $WAN_MAC"
...
if ! command -v brctl &>/dev/null; then
info "Installing bridge-utils..."
@@ -179,13 +190,6 @@ if ! command -v brctl &>/dev/null; then
info "bridge-utils installed."
fi
...
info "Installing xblp-bridge-rollback.service..."
install -m 644 "$SCRIPT_DIR/xblp-bridge-rollback.service" \
@@ -328,6 +331,7 @@ info "Writing /etc/network/interfaces.d/br0.conf ..."
sed \
-e "s/__WAN_IFACE__/${WAN_IFACE}/g" \
-e "s/__LAN_IFACE__/${LAN_IFACE}/g" \
    -e "s/__WAN_MAC__/${WAN_MAC}/g" \
...
# ── Nftables sanity note ───────────────────────────────────────────────────────
@@ -339,16 +343,7 @@ if ! nft list chain inet xblp forward &>/dev/null 2>&1; then
warn ""
fi
...
info "Network will restart in 3 seconds. This SSH session will disconnect."
printf '\n'
@@ -360,14 +355,11 @@ warn "If you cannot reconnect within ${CONFIRM_MINUTES} minutes, the device will
warn "reboot itself and restore the previous network configuration."
hr
